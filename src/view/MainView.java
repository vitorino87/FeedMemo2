	...
	protected void onStop(){
		...
		try{
			...
			if(mc.getTipoDeQuery()==4 || mc.getTipoDeQuery()==-1)
				...
				carregarTemp:{ //esse trecho faz parte da funcionalidade que faz o app carregar a ideia do �ltimo acesso
					gd.guardarEstado("temp"+mc.getTag(), mc.getCurrentId(), this); //serve para guardar a posi��o de cada tag
					break carregarTemp;
				}
		}catch(Exception e){
			e.printStackTrace();
	...
	protected void onStart(){
		...
		try{
			...
			if(mc.getTipoDeQuery()==2 && mc.getMaxId()!=-1 && mc.getMinId()!=-1 || mc.getTipoDeQuery()==3
				...
				//}		
				carregarTemp:{ //esse trecho faz parte da funcionalidade que faz o app carregar a ideia do �ltimo acesso
					if(mc.getTipoDeQuery()!=3 && gd.restaurarEstado("temp"+mc.getTag(), this)!=-1){     //este trecho serve para recuperar a posi��o dessa �ltima tag
